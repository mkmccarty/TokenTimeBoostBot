...
func TestContractHelpersDesignationAndDefaulting(t *testing.T) {
	contract := &Contract{
		...
		CustomOrderLines: []string{"<ROLE", "<IHR"},
	...
	// Custom Order with <ROLE should place u1 (non-helper main) first, despite u2 and guest1 having higher IHR
	sorted := sortCustomRemaining(contract, contract.Order, contract.CustomOrderLines, false)
	if sorted[0] != "u1" {
	...
	sortedAfterClear := sortCustomRemaining(contract, contract.Order, contract.CustomOrderLines, false)
